...
<head>
    ...
    <title>All Batch 2022</title> <!-- ############ TO BE CHANGED ############# -->
...
<body>
    <div class="content">
        <div class="header">
            <h2>All Batch 2022</h2> <!-- ############ TO BE CHANGED ############# -->
            ...
            <div class="total-payment-details">
                ...
                <div class="details-box">
                    <select id="batch-filter">
                        ...
                        <option value="BBA22">BBA</option>
                        <option value="BCA22">BCA</option>
                        <option value="BSC22">B.Sc CS(H)</option>
                    </select>
                </div>
            </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            const selectedYear = localStorage.getItem('selectedYear2022');
            if (selectedYear) {
        ...
        function selectYear(year) {
            localStorage.setItem('selectedYear2022', year); //############ TO BE CHANGED #############
            fetchData(year);
            ...
            fetch('batch-payment-data.php')
                ...
                .then(data => {
                    const filteredData = data.filter(row => {
                        const rollNo = row.ID;
                        return rollNo.includes("03BCA22") || rollNo.includes("03BSC22") || rollNo.includes("03BBA22");
                    });
        ...
        function populateTable(data, year) {
            ...
            data.forEach(row => {
                ...
                if (totalAmount === 0) {
                    status = "Outstanding";
                } else if (firstInstallment >= (totalAmount/2) && secondInstallment == 0 && thirdInstallment == 0 && fourthInstallment == 0) {
                    status = "Clear";
                    if ((secondInstallment + thirdInstallment + fourthInstallment) == (totalAmount/2)) {
                        status = "Clear";
                    }
            ...
            <td>${status}</td>
            <td><button onclick="updateData('${row.ID}', '${year}')" class="update-btn last-column">Update</button></td>
        `;
        ...
        function exportTableToExcel() {
            ...
            XLSX.writeFile(workbook, '2022-Batch-Data.xlsx'); //############ TO BE CHANGED #############
        }
    </script>
</body>
